...
fn alias_lookup_does_not_embed_nick_in_events() {
    ...
    let (status, raw) = call(&state, "POST", "/v1/alias", "{\"nick\":\"@Alice_01\"}");
    assert_eq!(status, 200, "{raw}");
...
fn owner_is_group_admin_and_contacts_list() {
    ...
    let (status, raw) = call(&state, "POST", "/v1/contacts", &add);
    assert_eq!(status, 200, "{raw}");
    assert!(raw.contains("Bob"));
    let (status, raw) = call(&state, "POST", "/v1/talk/group", "{\"name\":\"ops\"}");
    assert_eq!(status, 200, "{raw}");
    let group: CircleBare = blazingly_json::from_str(&raw).expect("group");
    assert_eq!(group.name, "ops");
    assert!(group.you_admin);
    let (status, raw) = call(&state, "GET", "/v1/chats", "");
    assert_eq!(status, 200, "{raw}");
...
fn public_room_post_omits_topic_from_event() {
    let state = Mutex::new(State::default());
    let created = create(&state);
    let (status, raw) = call(&state, "POST", "/v1/rooms/join", "{\"topic\":\"Privacy\"}");
    assert_eq!(status, 200, "{raw}");
    let (status, raw) =
        call(&state, "POST", "/v1/rooms/post", "{\"topic\":\"privacy\",\"text\":\"hello room\"}");
    assert_eq!(status, 200, "{raw}");
    let (status, raw) = call(&state, "GET", "/v1/account/history", "");
    assert_eq!(status, 200, "{raw}");
    assert!(!raw.to_ascii_lowercase().contains("privacy"));
    let (status, raw) = call(&state, "GET", "/v1/rooms", "");
    assert_eq!(status, 200, "{raw}");
